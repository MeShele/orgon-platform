...
| `ORGON_PLATFORM_MASTER_KEY` | only if `/platform/*` is in use | 32+ bytes random hex (`openssl rand -hex 32`). **Single shared secret** authenticating the `/platform/*` surface — today used by asystem-core's edge layer to self-provision Orgon merchants via `POST /platform/merchants`. Compromise blast-radius is "anyone can create / list merchants until rotated"; rotate by flipping this env var and reissuing to the caller, no DB migration, no merchant downtime. Unset → `/platform/*` returns `503` with a clear hint (intentional — the surface is opt-in). |
| `FINSUPERVISORY_SAR_BACKEND` | optional | `manual_export` (default) / `email` / `api_v1` (stub) / `dryrun`. Picks the active SAR submission channel. Surfaced to the dashboard via `GET /api/v1/compliance/sar/config` so the compliance officer sees the active mode at a glance. |
| `FINSUPERVISORY_SAR_EMAIL` | required when backend=email | Финнадзор address SAR submissions are auto-sent to. Verify with the regulator before flipping production to `email` mode. |
| `FINSUPERVISORY_SAR_CC` | optional | Compliance officer's own address — they keep a copy of every SAR sent on their side. Comma-separated for multiple recipients. |
| `TX_UNCERTAIN_TIMEOUT_MINUTES` | optional | Default `10`. Threshold at which a `signed`-without-`tx_hash` tx fires `transaction.uncertain` (non-terminal preview event). Garbage values fall back to default. Pairs with `TX_FAILED_TIMEOUT_HOURS` (default 24h) which fires the terminal `transaction.failed`. |
