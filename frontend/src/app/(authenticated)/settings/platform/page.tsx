  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-3">
        <Icon icon="solar:settings-bold" className="text-2xl text-indigo-500" />
        <div>
          <h1 className="text-2xl font-bold text-slate-900 dark:text-white">Настройки платформы</h1>
          <p className="text-sm text-slate-500 dark:text-slate-400">Глобальные настройки, брендинг, домены</p>
        </div>
      </div>
  ...
  return (
    <div className="space-y-4">
      {[
        ...
        <div key={s.title} className="rounded-xl border border-slate-200 bg-white p-5 dark:border-slate-800 dark:bg-slate-900 flex items-center gap-4 hover:border-indigo-300 dark:hover:border-indigo-700 transition-colors cursor-pointer">
          <Icon icon={s.icon} className="text-2xl text-slate-400" />
          <div>
...
function BrandingSection({ notify }: { notify: (t: "success" | "error", m: string) => void }) {
  ...
  return (
    <div className="rounded-xl border border-slate-200 bg-white p-6 dark:border-slate-800 dark:bg-slate-900 space-y-4">
      <h3 className="text-lg font-bold text-slate-900 dark:text-white">Брендинг</h3>
      {form && (
        <>
          <div>
          ...
          <div>
            <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1">URL логотипа</label>
            <input value={form.logo_url} onChange={(e) => setForm({ ...form, logo_url: e.target.value })}
              placeholder="https://..." className="w-full rounded-lg border border-slate-300 bg-white px-3 py-2 text-sm dark:border-slate-600 dark:bg-slate-800 dark:text-white" />
          </div>
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1">Основной цвет</label>
            ...
            <div>
              <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1">Дополнительный цвет</label>
              <div className="flex gap-2 items-center">
...
function DomainsSection({ notify }: { notify: (t: "success" | "error", m: string) => void }) {
  ...
  return (
    <div className="rounded-xl border border-slate-200 bg-white p-6 dark:border-slate-800 dark:bg-slate-900 space-y-4">
      <h3 className="text-lg font-bold text-slate-900 dark:text-white">Кастомные домены</h3>
...
function EmailsSection({ notify }: { notify: (t: "success" | "error", m: string) => void }) {
  ...
  return (
    <div className="rounded-xl border border-slate-200 bg-white p-6 dark:border-slate-800 dark:bg-slate-900 space-y-4">
      <h3 className="text-lg font-bold text-slate-900 dark:text-white">Email шаблоны</h3>
      {Array.isArray(templates) && templates.map((tpl: any) => (
        <div key={tpl.type} className="rounded-lg border border-slate-200 p-4 dark:border-slate-700">
          <div className="flex items-center justify-between mb-2">
            <span className="font-medium text-slate-900 dark:text-white">{tpl.type}</span>
          ...
          {editing === tpl.type ? (
            <div className="space-y-3">
              <input value={editForm.subject} onChange={(e) => setEditForm({ ...editForm, subject: e.target.value })}
                placeholder="Тема" className="w-full rounded-lg border border-slate-300 bg-white px-3 py-2 text-sm dark:border-slate-600 dark:bg-slate-800 dark:text-white" />
              <textarea value={editForm.body} onChange={(e) => setEditForm({ ...editForm, body: e.target.value })}
                rows={6} placeholder="Тело письма" className="w-full rounded-lg border border-slate-300 bg-white px-3 py-2 text-sm dark:border-slate-600 dark:bg-slate-800 dark:text-white" />
              <div className="flex gap-2">
            ...
            <p className="text-sm text-slate-500 dark:text-slate-400">{tpl.subject || "Не настроено"}</p>
          )}
        </div>
      ))}
    </div>
