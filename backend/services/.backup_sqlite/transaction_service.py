...
class TransactionService:
    ...
    async def get_transaction(self, unid: str) -> dict | None:
        ...
        tx = self._db.fetchone("SELECT * FROM transactions WHERE unid = ?", (unid,))
        if tx:
            sigs = self._db.fetchall(
                "SELECT * FROM tx_signatures WHERE tx_unid = ?", (unid,)
            )
            tx["signatures"] = sigs
        return tx
    ...
    async def sign_transaction(self, tx_unid: str) -> dict:
        ...
        self._db.execute(
            "UPDATE transactions SET status = 'signed', updated_at = ? WHERE unid = ?",
            (datetime.now(timezone.utc).isoformat(), tx_unid),
        )
    ...
    async def reject_transaction(self, tx_unid: str, reason: str = "") -> dict:
        ...
        self._db.execute(
            "UPDATE transactions SET status = 'rejected', updated_at = ? WHERE unid = ?",
            (datetime.now(timezone.utc).isoformat(), tx_unid),
        )
        ...
        for tx in txs:
            ...
            self._db.execute(
                ...
                (tx.id, tx.tx, tx.token, tx.token_name, tx.to_addr,
                 tx.value, tx.value_hex, tx.unid, tx.init_ts, tx.min_sign,
            ...
            if tx.wait:
                for sig in tx.wait:
                    if sig.get("ecaddress"):
                        self._db.execute(
                            """INSERT OR IGNORE INTO tx_signatures
                               (tx_unid, ec_address, sig_type) VALUES (?, ?, 'wait')""",
            ...
            if tx.signed:
                for sig in tx.signed:
                    if sig.get("ecaddress"):
                        self._db.execute(
                            """INSERT OR REPLACE INTO tx_signatures
                               (tx_unid, ec_address, sig_type, ec_sign) VALUES (?, ?, 'signed', ?)""",
                            (tx.unid, sig["ecaddress"], sig.get("ecsign")),
                        )

        self._db.execute(
            "INSERT OR REPLACE INTO sync_state (key, value, updated_at) VALUES (?, ?, ?)",
            ("transactions_synced", now, now),
        )
