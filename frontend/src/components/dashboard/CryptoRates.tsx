...
export function CryptoRates() {
  ...
  return (
    <div className="rounded-xl border border-border bg-white p-4 shadow-sm dark:border-border dark:bg-card/40">
      <div className="flex items-center gap-2 mb-3">
        <Icon icon="solar:chart-2-bold" className="text-lg text-primary" />
      ...
      {error && !isLoading && (
        <div className="text-xs text-muted-foreground text-center py-4">{t("noData")}</div>
      )}
        ...
        <div className="space-y-2">
          {rates.slice(0, 10).map((rate, i) => {
            ...
            return (
              <div
                key={`${symbol}-${i}`}
                className="flex items-center justify-between py-1.5 border-b border-slate-100 dark:border-border last:border-0"
              >
