        ...
        {profile && (
          <>
              ...
              <div className="p-4 sm:p-6">
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-sm text-muted-foreground mt-1">Управление паролем и настройками безопасности</p>
                  </div>
                  <button
            ...
            {sessions && sessions.length > 0 && (
              <Card>
                <div className="p-4 sm:p-6">
                  <h3 className="text-base font-semibold text-foreground mb-4">Активные сессии</h3>
                  <div className="space-y-3">
                    {sessions.map((s) => (
                      <div key={s.id} className="flex items-start justify-between rounded-lg border border-border bg-muted/50 p-3">
                        <div className="flex items-center gap-3">
                          <Icon icon="solar:monitor-linear" className="text-xl text-muted-foreground" />
                          <div>
                            ...
                            <p className="text-xs text-muted-foreground">{s.ip_address} • Последняя активность: {new Date(s.last_active).toLocaleString('ru-RU')}</p>
                          </div>
                        </div>
                        {!s.is_current && (
                          <button
