    ...
    <>
      ...
      <div className={pageLayout.container}>
        ...
        {activeTab === "onramp" && (
          <Card>
            <div className="p-6 space-y-4">
              <h3 className="text-lg font-semibold text-foreground">Покупка криптовалюты за фиат</h3>
              ...
              {rateLoading && <p className="text-sm text-muted-foreground">Загрузка курса...</p>}
        ...
        {activeTab === "offramp" && (
          <Card>
            <div className="p-6 space-y-4">
              <h3 className="text-lg font-semibold text-foreground">Продажа криптовалюты за фиат</h3>
              <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
                <div>
                  <label className="mb-1 block text-sm text-muted-foreground">Сумма крипто</label>
        ...
        {activeTab === "accounts" && (
          <div className="space-y-4">
            <Card>
              <div className="p-6 space-y-4">
                <h3 className="text-lg font-semibold text-foreground">Добавить банковский счёт</h3>
                <div className="grid gap-4 md:grid-cols-3">
                  <div>
                    <label className="mb-1 block text-sm text-muted-foreground">IBAN</label>
                    <Input value={newIban} onChange={(e) => setNewIban(e.target.value)} placeholder="KG00XXXX..." />
                  </div>
                  <div>
                    <label className="mb-1 block text-sm text-muted-foreground">Название банка</label>
                    <Input value={newBankName} onChange={(e) => setNewBankName(e.target.value)} placeholder="Optima Bank" />
                  </div>
                  <div>
                    <label className="mb-1 block text-sm text-muted-foreground">Имя владельца</label>
                  ...
                  {accounts.map((a) => (
                    <Card key={a.id}>
                      <div className="p-4 space-y-2">
                        ...
                        <p className="text-xs text-muted-foreground font-mono">{a.iban}</p>
                        <p className="text-sm text-muted-foreground">{a.holder_name}</p>
                      </div>
                    </Card>
                  ))}
                </div>
                    ...
                    <Card key={tx.id}>
                      <div className="p-4 space-y-2">
                        <div className="flex items-center justify-between">
                          <span className="font-medium text-foreground">{tx.type === "onramp" ? "Покупка" : "Продажа"}</span>
                          <Badge variant={statusColors[tx.status] as any}>{statusLabels[tx.status]}</Badge>
                        </div>
                        <p className="text-sm text-muted-foreground">{tx.amount} {tx.currency}</p>
                        {tx.crypto_amount && <p className="text-sm text-muted-foreground">{tx.crypto_amount} {tx.crypto_currency}</p>}
